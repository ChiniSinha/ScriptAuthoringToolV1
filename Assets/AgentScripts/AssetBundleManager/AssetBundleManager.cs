...
namespace AssetBundles
{
    ...
    public class AssetBundleManager : MonoBehaviour
    {
        ...
        private static readonly Dictionary<string, LoadedAssetBundle> m_LoadedAssetBundles =
            new Dictionary<string, LoadedAssetBundle>();

        private static readonly Dictionary<string, WWW> m_DownloadingWWWs = new Dictionary<string, WWW>();
        private static readonly Dictionary<string, string> m_DownloadingErrors = new Dictionary<string, string>();

        private static readonly List<AssetBundleLoadOperation> m_InProgressOperations =
            new List<AssetBundleLoadOperation>();
        ...
        public static AssetBundleLoadManifestOperation Initialize(string manifestAssetBundleName)
        {
            ...
            AssetBundleLoadManifestOperation operation = new AssetBundleLoadManifestOperation(manifestAssetBundleName,
        ...
        protected static string RemapVariantName(string assetBundleName)
        {
            ...
            for (int i = 0; i < bundlesWithVariant.Length; i++)
            {
                ...
                int found = Array.IndexOf(m_ActiveVariants, curSplit[1]);
        ...
        protected static void LoadDependencies(string assetBundleName)
        {
			...
			Debug.Log("Finding dependencies for " + assetBundleName + "...");
            ...
            for (int i = 0; i < dependencies.Length; i++)
            {
				Debug.Log("Loading dependency " + dependencies[i]);
        ...
        public static void UnloadAssetBundle(string assetBundleName)
        {
            ...
            //RagLog.Log(m_LoadedAssetBundles.Count + " assetbundle(s) in memory before unloading " + assetBundleName);

            UnloadAssetBundleInternal(assetBundleName);
            UnloadDependencies(assetBundleName);

            //RagLog.Log(m_LoadedAssetBundles.Count + " assetbundle(s) in memory after unloading " + assetBundleName);
        }
        ...
        private void Update()
        {
            ...
            KeyValuePair<string, WWW> keyValue;
            WWW download;
            AssetBundle bundle;
            for (var iter = m_DownloadingWWWs.GetEnumerator(); iter.MoveNext();)
            {
                keyValue = iter.Current;
                ...
                if (download.error != null)
                {
                    m_DownloadingErrors.Add(keyValue.Key,
                        string.Format("Failed downloading bundle {0} from {1}: {2}", keyValue.Key, download.url,
                            download.error));
                    _completedDownloads.Add(keyValue.Key);
                    continue;
                }
                ...
                if (download.isDone)
                {
                    ...
                    //RagLog.Log("Downloading " + keyValue.Key + " is done at frame " + Time.frameCount);
            ...
            for (int i = 0; i < _completedDownloads.Count; i++)
            {
                m_DownloadingWWWs[_completedDownloads[i]].Dispose();
                m_DownloadingWWWs.Remove(_completedDownloads[i]);
            }
        ...
        private List<string> _completedDownloads = new List<string>();
#if UNITY_EDITOR
