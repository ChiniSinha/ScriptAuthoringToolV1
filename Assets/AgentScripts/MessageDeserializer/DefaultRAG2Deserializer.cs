...
public class DefaultRAG2Deserializer : IMessageDeserializer
{
    public BaseCommand DeserializeActionXml(XmlNode node)
    {
        ...
        {
           // case "debug":
            //    return DeserializeDebugBlock(node);
...
/*
            case "widget":
                return DeserializeWidgetBlock(node);
            ...
            case "video":
                return DeserializeVideoBlock(node);
            ...
            case "print":
                return DeserializePrintBlock(node);
*/
            case "speech":
...
/*
            case "pausescreen":
                return DeserializePauseScreenBlock(node);
...
/*
            ...
            case "environment":
                return DeserializeEnvironmentBlock(node);
                */
        }
...
/*
    private BaseCommand DeserializePauseScreenBlock(XmlNode node)
    {
        return new ShowPauseScreenCommand(node.AttributeCaseInsensitive("title"),
            node.AttributeCaseInsensitive("buttons").Split(new[] {'|'}, StringSplitOptions.RemoveEmptyEntries));
    }

    private BaseCommand DeserializeEnvironmentBlock(XmlNode node)
    {
        return new ChangeEnvironmentCommand(node.SafeGetAttribute("name"), true);
    }
...
/*
    private BaseCommand DeserializeDebugBlock(XmlNode node)
    {
        if (node.AttributeCaseInsensitive("command").Equals("state", StringComparison.CurrentCultureIgnoreCase))
        {
            return new DebugTextCommand(node.AttributeCaseInsensitive("info"));
        }
        return new DebugTextCommand(node.AttributeCaseInsensitive("info"));
    }
    ...
    private BaseCommand DeserializeTableDisplayBlock(XmlNode node)
    {
        ...
        return new DisplayTableCommand(tableContents, new List<string>(buttonList.Split('|')));
    }

    private BaseCommand DeserializeWidgetBlock(XmlNode node)
    {
        string cmd = node.AttributeCaseInsensitive("cmd");
        if (!string.IsNullOrEmpty(cmd) && cmd.Equals("clear", StringComparison.CurrentCultureIgnoreCase))
        {
            return new ClearWidgetCommand();
        }

        string widgetType = node.AttributeCaseInsensitive("type");
        if (widgetType.Equals("pedigree", StringComparison.CurrentCultureIgnoreCase))
        {
            return PedigreeMessageParser.DeserializeCommand(node);
        }

        if (widgetType.Equals("mhtdl", StringComparison.CurrentCultureIgnoreCase))
        {
            return MhtdlMessageParser.DeserializeCommand(node);
        }
...
//TODO: Add video preview in
/*
    private BaseCommand DeserializeVideoBlock(XmlNode node)
    {
        if (node.AttributeCaseInsensitive("cmd").Equals("clear", StringComparison.CurrentCultureIgnoreCase))
        {
            return new HideVideoCommand();
        }
        ...
        return new ShowVideoCommand(node.AttributeCaseInsensitive("url"), menuButtons);
    }
 */
/*
...
/*
    ...
    protected virtual BaseCommand DeserializePrintBlock(XmlNode node)
    {
        string url = node.AttributeCaseInsensitive("url");
        if (string.IsNullOrEmpty(url))
        {
            return new PrintContentCommand(node.InnerText);
        }
        if (url.EndsWith("png") || url.EndsWith("jpg"))
        {
            return new OpenPrintableImageCommand(url, "");
        }
        return new PrintPageCommand(url);
    }
*/
    ...
    private BaseCommand DeserializeEyebrowsBlock(XmlNode expressionNode)
    {
        ...
        switch (expression)
        {
            ...
            case "DOWN":
                return new SetExpressionCommand("EYEBROWS_DOWN");
        }
